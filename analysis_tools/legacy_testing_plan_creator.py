    ...
    for index, junction in enumerate(junctions):
        for config_folder in config_folders[index]:
            for random_seed_value in random_seed_values:
                for spawn_time_delta_value in spawn_time_delta_values:
                    plan[str(uid)] = {
                        "RunUID": "Legacy_uneven_spawning_control_method_" + control_method.replace(" ", "_") + "_junction_" + junction[:-5] + "_seed_" + str(random_seed_value) + "_spawn_rate_" + str(round(60/spawn_time_delta_value, 2)) + "cpm",
                        "RunType": control_method,
                        "Junction": junction,
