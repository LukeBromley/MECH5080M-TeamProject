...
class Simulation:
    ...
    def main(self):
        ...
        while True:
            if random.random() > 0.990:
                self.add_vehicle(random.randint(0,1))
            ...
            for vehicle in self.model.get_vehicles():
                ...
                if (vehicle.lane_ghost is None) and (vehicle.is_ghost is False) and vehicle.get_route_distance_travelled() > 5:
                    path, path_distance = (self.model.get_route(vehicle.route_uid)).get_path_and_path_distance_travelled(vehicle.get_route_distance_travelled())
                    s_value = path.get_s(path_distance)
                    new_path = self.model.paths[int(not(path.uid))]
                    distance_travelled = new_path.get_arc_length_from_s(s_value)
                    vehicle.begin_lane_change(distance_travelled)
                    self.model.vehicles.append(vehicle.lane_ghost)
    ...
    def get_object_ahead(self, vehicle_uid):
        ...
        this_path, this_vehicle_path_distance_travelled = this_route.get_path_and_path_distance_travelled(
            this_vehicle.get_route_distance_travelled())

        # Search the current path
        min_path_distance_travelled = float('inf')
        for that_vehicle in self.model.get_vehicles():
            that_path, that_vehicle_path_distance_travelled = self.model.get_route(
                that_vehicle.route_uid).get_path_and_path_distance_travelled(that_vehicle.get_route_distance_travelled())
        ...
        this_route_path_uids = self.model.get_route(
            this_vehicle.route_uid).get_path_uids()
        ...
        for path_uid in path_uids_ahead:
            ...
            if object_ahead is not None:
                ...
                distance_travelled_offset += self.model.get_path(
                    path_uid).get_length()
                continue
...
if __name__ == "__main__":
    sim = Simulation(os.path.join(
        ROOT_DIR, "Junction_Designs", "Lane_Changing.junc"))
    sim.run()
