...
class TrainedModelTester:
    ...
    def run_testing_runs(self) -> None:
        ...
        for run in self.testing_runs:
            ...
            backup_mean_average, backup_standard_deviation, backup_maximum, backup_time, backup = self.run_testing_run(run["RunUID"], run["RunType"], run["Junction"], run["SimulationConfig"], run["MachineLearningModel"], run["Steps"], run["HumanDriversVisible"])
    ...
    def run_testing_run(self, run_uid, run_type, junction_file_name, sim_config_file_name, ml_model_folder_name, steps, human_drivers_visible):
        ...
        if run_type.lower() == "junction":
            ...
            machine_learning.test(steps, ml_model_model_file_path, human_drivers_visible)
